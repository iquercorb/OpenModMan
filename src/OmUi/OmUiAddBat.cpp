...
bool OmUiAddBat::_onBcOk()
{
  ...
  if(!this->_pCtx->batAdd(bat_name, loc_uuid, loc_hash_list)) {
    ...
    Om_dlgBox_okl(this->_hwnd, L"New Installation Batch", IDI_ERR,
...
void OmUiAddBat::_onResize()
{
  ...
  this->_setItemPos(IDC_CB_LOC, 10, 69, this->cliUnitX()-20, 12);
  // Not-Installed label
  this->_setItemPos(IDC_SC_LBL03, 10, 85, 150, 9);
  // Not-Installed ListBox
  this->_setItemPos(IDC_LB_EXC, 10, 95, half_w-30, this->cliUnitY()-130);
  // Add and Rem buttons
  ...
  // Installed ListBox
  this->_setItemPos(IDC_LB_INC, half_w, 95, half_w-30, this->cliUnitY()-130);
  // Up and Down buttons
  this->_setItemPos(IDC_BC_UP, this->cliUnitX()-28, helf_h+15, 16, 15);
  this->_setItemPos(IDC_BC_DN, this->cliUnitX()-28, helf_h+30, 16, 15);
