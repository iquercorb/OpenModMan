...
void OmUiPropBatStg::_onResize()
{
  ...
  this->_setItemPos(IDC_EC_INP01, 50, 25, this->cliUnitX()-100, 13);

  // Batch Configuration Label
  this->_setItemPos(IDC_SC_LBL02, 50, 45, 240, 9);
  // Location list ComboBox
  this->_setItemPos(IDC_CB_LOC, 50, 55, this->cliUnitX()-100, 12);
  ...
  this->_setItemPos(IDC_LB_EXC, 50, 80, half_w-70, this->cliUnitY()-95);
  // Add and Rem buttons
  ...
  // Installed ListBox
  this->_setItemPos(IDC_LB_INC, half_w, 80, half_w-70, this->cliUnitY()-95);
  // Up and Down buttons
  this->_setItemPos(IDC_BC_UP, this->cliUnitX()-68, half_h+15, 16, 15);
  this->_setItemPos(IDC_BC_DN, this->cliUnitX()-68, half_h+30, 16, 15);
}
...
void OmUiPropBatStg::_onRefresh()
{
  ...
  if(unavail) {
    ...
    // query user
    if(Om_dlgBox_yn(this->_hwnd, L"Installation Batch Properties", IDI_QRY,
